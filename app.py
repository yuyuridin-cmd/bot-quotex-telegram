import os, threading, time, traceback, asyncio, random
from flask import Flask, send_from_directory, jsonify, request
from flask_cors import CORS

# --- IMPORT QUOTEX CON 2 INTENTOS ---
Quotex = None
IMPORT_ERROR = ""
try:
    from quotexapi.stable_api import Quotex
    print(">>> OK con quotexapi")
except Exception as e:
    IMPORT_ERROR = str(e)
    try:
        from pyquotex.stable_api import Quotex
        print(">>> OK con pyquotex")
    except Exception as e2:
        IMPORT_ERROR += " | " + str(e2)
        print(f">>> FALLO IMPORT: {IMPORT_ERROR}")

HAS_QUOTEX = Quotex is not None
PORT = 10000

# Config con tus limites
config = {
    "saldo": 0,
    "base": 0,
    "wins": 0,
    "loss": 0,
    "monto_pct": 6,
    "limite_gana": 10,      # Se para si gana +10
    "limite_perdida": 10,   # Se para si pierde -10
    "modo": "DEMO",
    "activo": False,
    "conectado": False,
    "email": "",
    ...
    while True:
        try:
            if client and config["conectado"]:
                ...
                # CONTROL AUTOMATICO DE LIMITES
                if config["activo"]:
                    gan = config["saldo"] - config["base"]
                    if gan >= config["limite_gana"]:
                        config["activo"] = False
                        config["msg"] = f"Meta alcanzada +${gan:.2f} - Bot parado"
                        print(config["msg"])
                    if gan <= -config["limite_perdida"]:
                        config["activo"] = False
                        config["msg"] = f"Stop loss -${abs(gan):.2f} - Bot parado"
                        print(config["msg"])
        except Exception as e:
...
def login():
    ...
    try:
        ...
        if not HAS_QUOTEX:
            return jsonify({"ok": False, "error": f"Falta libreria quotexapi: {IMPORT_ERROR}"})

        print(f"Conectando {email} en {modo}...")
        config["msg"] = "Conectando..."
...
def start():
    config["activo"] = True
    config["base"] = config["saldo"] # reinicia base para el limite del dia
